...
ALTER TABLE `ROUTES` ALTER COLUMN 
...
ALTER TABLE PRICE ADD COLUMN PRICE INT ;
...
SELECT P.GENDER , COUNT(P.PASSENGER_NAME) FROM 
PASSENGER P JOIN ROUTES R ON P.ROUTE_ID = R.ROUTE_ID
WHERE R.DISTANCE >= 600
GROUP BY P.GENDER ;
...
WHERE p.BUS_TYPE ='Sleeper';


SELECT PRICE FROM PRICE ORDER BY PRICE ASC LIMIT 1;
...
SELECT PASSENGER_NAME FROM PASSENGER p 
WHERE p.PASSENGER_NAME like 'S%';
...
SELECT PA.PASSENGER_NAME , R.BOARDING_CITY , R.DESTINATION_CITY ,PR.SEASON ,PR.BUS_TYPE , PR.PRICE FROM 
PASSENGER PA JOIN ROUTES R ON PA.ROUTE_ID = R.ROUTE_ID 
JOIN PRICE PR ON R.ROUTE_ID = PR.ROUTE_ID 
WHERE PR.SEASON = 'Festive';
...
JOIN PRICE PR ON R.ROUTE_ID = PR.ROUTE_ID 
WHERE R.DISTANCE = 1000 AND PR.BUS_TYPE = 'Sitting';
...
SELECT PA.PASSENGER_NAME , R.DESTINATION_CITY AS `BOARDING CITY`, R.BOARDING_CITY AS `DESTINATION CITY`, R.DISTANCE , PR.BUS_TYPE ,PR.PRICE 
FROM PASSENGER PA
JOIN ROUTES R ON PA.ROUTE_ID = R.ROUTE_ID 
JOIN PRICE PR ON R.ROUTE_ID = PR.ROUTE_ID 
